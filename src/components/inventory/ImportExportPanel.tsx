...
export function ImportExportPanel() {
  ...
  const [action, setAction] =
    useState<ImportAction>("update_all");
  ...
  const doImport = useMutation({
    mutationFn: async () => {
      ...
      if (
        existingRows.length &&
        wantsProductPatch(action)
      ) {
        totalOperations += existingChunks;
      }

      if (newRows.length) {
        totalOperations += newChunks;
        totalOperations += newChunks;
      }
      ...
      /*
       * IMPORTANTE:
       * El stock ya NO se escribe directamente en
       * `inventory`.
       *
       * Se actualiza mediante adjust_stock(),
       * que en Lula OS está conectado al
       * shared_inventory.
       */
      ...
      if (newRows.length) {
        ...
        for (const row of newRows) {
          if (
            failedRows.has(row.row)
          ) {
            continue;
          }

          const productId =
            newIdByRow.get(row.row);
  ...
  if (!isManager) {
    return (
      <p className="text-sm text-muted-foreground">
        Solo managers pueden importar o
        exportar inventario.
      </p>
    );
  }
